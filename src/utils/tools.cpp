...
QImage Tools::LinearLevelTransformation(const QImage &origin, double _a, double _b)
{
    ...
    for (int x = 0; x < newImage.width(); x++) {
        for (int y = 0; y < newImage.height(); y++) {
            ...
            grayLevel1 = pow((pow(oldColor.red(), 2.2) * 0.2973 + pow(oldColor.green(), 2.2) * 0.6274 + pow(oldColor.blue(), 2.2) * 0.0753), (1 / 2.2));
            qInfo() << "grayLevel1 - grayLevel" << grayLevel1 - grayLevel;
...
{
    QImage newImage(origin.width(), origin.height(),
...
{
    QImage baseImage = QImage(":/img/metal.png");
    qInfo() << "baseImage->save " << baseImage.save("baseImage.png");
    QImage darkImage = Tools::Brightness(-100, origin);
    darkImage.save("darkImage.png");
 ...
 *                                转置变换
 * **************************************************************************/
QImage Tools::Transpose(const QImage &origin)
{
    int width = origin.width();
    int height = origin.height();
